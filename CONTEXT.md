...
- [x] Step 31: clinic.css contrast fixes — luxe #8B6914→#7A5500 on light bg, footer logo →#C9A84C on #0A0A0A; premium nav span/btn bg/tags/stars #00B4D8→#006D87 on white/light bg (ratio 2.5→5-7:1); #00B4D8 kept on dark backgrounds (hero, CTA, footer) where ratio is OK — luxe #8B6914→#7A5500 on light bg; premium btn/tags/stars/nav span #F59E0B→#92600A on white; vibrant nav/footer span #7B2FF7→#9D5FF5 on dark bg; F59E0B kept on dark backgrounds (hero, CTA, footer) where ratio is OK — preload fetchpriority="high"; tu-luxe-hero min-height:88vh→height:88svh+max-height:900px (stable at first paint); tu-luxe-nav__cta + site-nav__links tags + tu-luxe-hero__badge added to inline critical CSS; main.js IntersectionObserver refactored to batch all DOM reads before writes (fixes 93ms forced reflow) — preload link gets fetchpriority="high"; bw-luxe-hero height:90vh→height:90svh+max-height:900px (stable at first paint, no reflow); bw-luxe-hero__img gets will-change:transform; bw-luxe-nav__cta + site-nav__links tag styles added to inline critical CSS (were unstyled at first paint causing CLS 0.003); all #8B6914 in beauty.css →#7A5500 (contrast fix) — id-luxe-footer__logo: #8B6914→#C9A84C on #0A0A0A; all #8B6914 luxe text →#7A5500 on light bg (#FAFAFA/#FAF8F5, ratio ≈5:1); #2ECC71 on light bg (nav logo span, services/temo tags, stars, btn bg) →#1A7A40 (ratio ≈5-7:1); #2ECC71 on dark bg (hero badge/title span, CTA title span, footer logo span) →#4ADE80 (ratio ≈6:1)
