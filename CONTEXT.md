...
- [x] Step 28: beauty.html + beauty.css CLS/LCP fixes — preload link gets fetchpriority="high"; bw-luxe-hero height:90vh→min-height:90svh + contain:layout style (eliminates vh CLS); bw-luxe-hero__img gets will-change:transform; bw-luxe-nav__cta added to inline critical CSS (was unstyled at first paint); all #8B6914 in beauty.css →#7A5500 (contrast fix, ratio 3.2→5:1 on #FAFAFA) — id-luxe-footer__logo: #8B6914→#C9A84C on #0A0A0A; all #8B6914 luxe text →#7A5500 on light bg (#FAFAFA/#FAF8F5, ratio ≈5:1); #2ECC71 on light bg (nav logo span, services/temo tags, stars, btn bg) →#1A7A40 (ratio ≈5-7:1); #2ECC71 on dark bg (hero badge/title span, CTA title span, footer logo span) →#4ADE80 (ratio ≈6:1)
